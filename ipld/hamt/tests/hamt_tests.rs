...
fn delete() {
    ...
    let mut hamt: Hamt<String, _> = Hamt::new(&store);
    ...
    let mut h2 = Hamt::<String, _>::load(&c, &store).unwrap();
    assert_eq!(h2.delete(&"foo".to_owned()).unwrap(), true);
    assert_eq!(h2.get::<_, ByteBuf>(&"foo".to_owned()).unwrap(), None);
...
fn set_delete_many() {
    ...
    let mut hamt: Hamt<String, _> = Hamt::new_with_bit_width(&store, 5);

    for i in 0..200 {
        hamt.set(format!("{}", i), i).unwrap();
    }
    ...
    for i in 200..400 {
        assert_eq!(hamt.delete(&format!("{}", i)).unwrap(), true);
    }
