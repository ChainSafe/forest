...
impl BasicBlock {
    ...
    fn new(&self, data: Vec<u8>) -> Self {
        // TODO replace SHA2256 with Blake2b when rust-cid updates multihash dependency
        let h = encode(Hash::SHA2256, data.as_ref()).unwrap();
        Self {
    ...
    fn new_blk_with_cid(&self, data: Vec<u8>, cid: Cid) -> Self {
        // TODO check cryptographic sum of buffer; see https://github.com/ipfs/go-block-format/blob/master/blocks.go#L45
