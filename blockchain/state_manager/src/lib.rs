    ...
    DB: BlockStore,
{
    ...
    pub async fn is_miner_slashed(&self, addr: &Address, state_cid: &Cid) -> Result<bool, Error> {
        let bs = Arc::clone(&self.bs);
        ...
        match ps.get_claim(bs.as_ref(), addr)? {
            Some(_) => Ok(false),
            None => Ok(true),
        }
    }
    /// Returns raw work address of a miner
    pub async fn get_miner_work_addr(
        &self,
        state_cid: &Cid,
        addr: &Address,
        ...
        let state = StateTree::new_from_root(bs.as_ref(), state_cid).map_err(Error::State)?;
        // Note: miner::State info likely to be changed to CID
        let addr = resolve_to_key_addr(&state, bs.as_ref(), &ms.info.worker)
    ...
    pub async fn get_power(
        ...
        if let Some(claim) = ps.get_claim(bs.as_ref(), addr)? {
