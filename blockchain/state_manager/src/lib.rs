...
use forest_interpreter::{
    check_message, fvm::ForestExterns, fvm::ForestKernel, fvm::ForestMachine, resolve_to_key_addr,
    BlockMessages, CircSupplyCalc, Heights, RewardCalc,
};
...
use fvm::machine::{Engine, NetworkConfig};
...
use fvm_shared::{DefaultNetworkParams, BLOCK_GAS_LIMIT};
...
pub struct SMLookbackWrapper<DB> {
    sm: Arc<StateManager<DB>>,
    tipset: Arc<Tipset>,
}

impl<DB> SMLookbackWrapper<DB>
where
