...
where
    DB: BlockStore,
{
    let mut miner_actor_state: State = state_manager.load_actor_state(&address, &st)?;
    let sector_set = miner_actor_state.compute_proving_set(&*state_manager.get_block_store())?;
