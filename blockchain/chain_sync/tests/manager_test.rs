...
fn schedule_tipset() {
    ...
    let tipset = Arc::new(Tipset::new(vec![header]).unwrap());
...
fn heaviest_different_chain() {
    let l_tipset = Arc::new(Tipset::new(vec![create_header(1, b"1", b"1")]).unwrap());
    let m_tipset = Arc::new(Tipset::new(vec![create_header(2, b"2", b"2")]).unwrap());
    let h_tipset = Arc::new(Tipset::new(vec![create_header(3, b"1", b"1")]).unwrap());
    let mut manager = SyncManager::default();
