...
use forest_shim::{econ::TokenAmount, BLOCK_GAS_LIMIT};
...
fn compute_next_base_fee(
    ...
    let mut delta: i64 = if epoch > smoke_height {
        (gas_limit_used / no_of_blocks as u64) as i64 - BLOCK_GAS_TARGET as i64
    } else {
    ...
    if delta.abs() as u64 > BLOCK_GAS_TARGET {
    ...
    for b in ts.blocks() {
        ...
        for m in msg1 {
            ...
            if !seen.contains(&m_cid) {
                total_limit += m.gas_limit as u64;
                seen.insert(m_cid);
            }
        }
        for m in msg2 {
