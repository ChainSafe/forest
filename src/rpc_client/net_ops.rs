...
pub async fn net_addrs_listen(auth_token: &Option<String>) -> Result<NetAddrsListenResult, Error> {
    call(NET_ADDRS_LISTEN, (), auth_token).await
}

pub async fn net_peers(auth_token: &Option<String>) -> Result<NetPeersResult, Error> {
    call(NET_PEERS, (), auth_token).await
}

pub async fn net_info(auth_token: &Option<String>) -> Result<NetInfoResult, Error> {
