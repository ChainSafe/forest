...
impl<DB: Blockstore + Send + Sync + 'static> ForestExterns2<DB> {
    pub fn new(
        // rand: impl Rand + 'static,
        // heaviest_tipset: Arc<Tipset>,
        epoch: ChainEpoch,
...
mod integration_tests {
    ...
    use crate::blocks::{ElectionProof, Ticket};
    use fvm_ipld_encoding::to_vec;
    use num_bigint::BigInt;

    use fvm_integration_tests::externs::externs_tests;
    use fvm_integration_tests::externs::Address as FvmAddress;
    use fvm_integration_tests::externs::Chain;
    use fvm_integration_tests::externs::Consensus;
    use fvm_integration_tests::externs::ConsensusFault;
    use fvm_integration_tests::externs::ConsensusFaultType;
    // use fvm_integration_tests::externs::Externs;

    use crate::db::MemoryDB;

    use super::Blockstore;
    use super::ForestExterns2;
    use crate::blocks::CachingBlockHeader;
    use crate::blocks::RawBlockHeader;
    use crate::chain::index::ChainIndex;
    use crate::networks::{Height, NetworkChain};
    ...
    impl<DB: Blockstore + Send + Sync + 'static> Consensus for ForestExterns2<DB> {
        // See https://github.com/filecoin-project/lotus/blob/v1.18.0/chain/vm/fvm.go#L102-L216 for reference implementation
        fn verify_consensus_fault(
            &self,
            h1: &[u8],
            ...
            if bh_1.cid() == bh_2.cid() {
                println!("blocks are the same (cid)");
                bail!("no consensus fault: submitted blocks are the same");
            }
            ...
            if self.chain_config.network == NetworkChain::Calibnet {
                ...
                let is_near_upgrade = |epoch| {
                    epoch > watermelonfix_height - finality
                        && epoch < watermelonfix_height + finality
                };
            ...
            // in the chain).
            if bh_2.epoch < bh_1.epoch {
                println!("block 2 must be earlier or equal");
                bail!(
                    "first block must not be of higher height than second: {:?}, {:?}",
                    bh_1.epoch,
                    bh_2.epoch
            ...
            // (a) double-fork mining fault
            if bh_1.epoch == bh_2.epoch {
                println!("double-fork mining fault");
                fault_type = Some(ConsensusFaultType::DoubleForkMining);
            };
            ...
            match fault_type {
                ...
                Some(fault_type) => {
                    ...
                    let ret = Some(ConsensusFault {
                        target: FvmAddress::new_id(bh_1.miner_address.id().unwrap()),
                        epoch: bh_2.epoch,
                        fault_type,
                    });
    ...
    impl<DB: Blockstore> Chain for ForestExterns2<DB> {
        fn get_tipset_cid(&self, epoch: ChainEpoch) -> anyhow::Result<Cid> {
            todo!()
        }
    }

    fn mock_consensus(epoch: i64) -> Box<dyn fvm_integration_tests::externs::Consensus> {
        let externs = ForestExterns2 {
            epoch,
            root: Cid::default(),
            chain_index: Arc::new(ChainIndex::new(Arc::new(MemoryDB::default()))),
    ...
    fn mock_block(
        ...
        let addr = Address::new_id(miner_id);
        let cid =
        ...
        let parents = if let Ok(non_empty) = nunny::Vec::new(cids) {
            non_empty.into()
        } else {
            crate::blocks::header::GENESIS_BLOCK_PARENTS.clone()
        };
        ...
        (to_vec(&header).unwrap(), header.cid().clone())
    }

    #[test]
    fn verify_consensus_fault_test() {
        externs_tests::verify_consensus_fault_test::<ForestExterns2<MemoryDB>, _, _>(
            mock_consensus,
            mock_block,
        );
    }
