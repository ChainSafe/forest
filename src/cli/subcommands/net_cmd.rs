...
impl NetCommands {
    pub async fn run(self, config: Config) -> anyhow::Result<()> {
        match self {
            Self::Listen => {
                let info = net_addrs_listen(Default::default(), &config.client.rpc_token)
                    .await
            ...
            Self::Info => {
                let info = net_info(Default::default(), &config.client.rpc_token)
                    .await
                    .map_err(handle_rpc_err)?;
            ...
            Self::Peers => {
                let addrs = net_peers(Default::default(), &config.client.rpc_token)
                    .await
                    .map_err(handle_rpc_err)?;
