...
pub async fn run_test_from_snapshot(path: &Path) -> anyhow::Result<()> {
    ...
    let mut run = false;
    let snapshot: RpcCallSnapshot = serde_json::from_slice(std::fs::read(path)?.as_slice())?;
    let db = Arc::new(MemoryDB::deserialize_from(
        BASE64_STANDARD.decode(&snapshot.db)?.as_slice(),
    )?);
...
mod tests {
    ...
    #[tokio::test]
    async fn rpc_regression_tests() -> anyhow::Result<()> {
        for path in std::fs::read_dir("/var/tmp/rpc-snapshots")?
            .filter_map(|entry| Some(entry.ok()?.path()))
            .filter(|entry| entry.is_file())
        {
            print!("Testing {} ...", path.display());
            run_test_from_snapshot(&path).await?;
            println!("  succeeded.");
        }
        Ok(())
    }
