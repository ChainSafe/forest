...
/// <`Eip155TransactionArgs`> is a legacy Ethereum transaction that uses the EIP-155 chain replay protection mechanism
...
/// For <`Eip155TransactionArgs`>, the digest that is used to create a signed transaction includes the `ChainID` but the serialised RLP transaction
...
impl EthLegacyEip155TxArgs {
    pub(crate) fn initialise_signature(
        &mut self,
        signature: &Signature,
        eth_chain_id: u64,
    ) -> anyhow::Result<()> {
        ...
        let valid_sig_len = calc_valid_eip155_sig_len(eth_chain_id);
            ...
            "Invalid signature length for EIP155 transaction: {}",
            signature.bytes().len()
        );
        ...
        let r = BigInt::from_bytes_be(Sign::Plus, &signature.bytes()[1..33]);
        let s = BigInt::from_bytes_be(Sign::Plus, &signature.bytes()[33..65]);
        let v = BigInt::from_bytes_be(Sign::Plus, &signature.bytes()[65..]);
...
fn validate_eip155_chain_id(eth_chain_id: EthChainId, v: &BigInt) -> anyhow::Result<()> {
    let derived_chain_id = derive_eip_155_chain_id(v)?;
        ...
        derived_chain_id
            .to_u64()
            .context("unable to convert derived chain to `u32`")?
            == eth_chain_id,
...
pub(super) fn calc_valid_eip155_sig_len(eth_chain_id: u64) -> (u64, u64) {
