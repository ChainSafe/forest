...
impl MarketStateExt for market::State {
    fn get_allocations_for_pending_deals(
        ...
        match self {
            ...
            Self::V18(s) => {
                let map = fil_actors_shared::v18::Map::<_, AllocationID>::load_with_bit_width(
                    &s.pending_deal_allocation_ids,
                    store,
                    fil_actors_shared::v18::HAMT_BIT_WIDTH,
                )?;
                map.for_each(|k, &v| {
                    let deal_id = fil_actors_shared::v18::parse_uint_key(k)?;
                    result.insert(deal_id, v);
                    Ok(())
                })?;
            }
