...
impl<BS: Blockstore> BalanceTableExt for BalanceTable<'_, BS> {
        ...
        F: FnMut(&Address, &TokenAmount) -> anyhow::Result<()>,
    {
        match self {
            ...
            Self::V18(t) => t.0.for_each(|address, escrow| {
                f(&address.into(), &escrow.into())
                    .map_err(|e| fil_actors_shared::v18::ActorError::unspecified(e.to_string()))
            })?,
            Self::V19(t) => t.0.for_each(|address, escrow| {
                f(&address.into(), &escrow.into())
                    .map_err(|e| fil_actors_shared::v19::ActorError::unspecified(e.to_string()))
            })?,
        };
        Ok(())
    }
