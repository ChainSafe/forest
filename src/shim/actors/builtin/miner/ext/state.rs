...
impl MinerStateExt for State {
    fn sectors(&self) -> &Cid {
        match self {
            ...
            State::V10(st) => &st.sectors,
            State::V11(st) => &st.sectors,
            State::V12(st) => &st.sectors,
            State::V13(st) => &st.sectors,
            State::V14(st) => &st.sectors,
            State::V15(st) => &st.sectors,
            State::V16(st) => &st.sectors,
        }
    }
    ...
    fn load_sectors_ext<BS: Blockstore>(
        ...
        match self {
            State::V8(st) => {
                ...
                sectors.amt.for_each(|i, info| {
                    match filter {
                        Some(s) if !s.get(i) => {}
                        _ => infos.push((i, info.clone().into())),
                    };
                    Ok(())
                })?;
                Ok(infos)
