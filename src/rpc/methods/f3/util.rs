...
const `F3`_PERMANENT_PARTICIPATING_MINER_IDS_ENV_KEY: &str =
    "FOREST_`F3`_PERMANENT_PARTICIPATING_MINER_ADDRESSES";

pub static `F3`_PERMANENT_PARTICIPATING_MINER_IDS: LazyLock<Option<HashSet<u64>>> =
...
fn get_f3_permanent_participating_miner_ids() -> Option<HashSet<u64>> {
    if let Ok(permanent_addrs) = std::env::var(`F3`_PERMANENT_PARTICIPATING_MINER_IDS_ENV_KEY) {
        let mut ids = HashSet::default();
        for addr_str in permanent_addrs.split(",") {
            let Ok(addr) = Address::from_str(addr_str.trim()) else {
                tracing::warn!(
                    "Failed to parse miner address {addr_str} set in {`F3`_PERMANENT_PARTICIPATING_MINER_IDS_ENV_KEY}"
                );
                continue;
            };
            let Ok(id) = addr.id() else {
                tracing::warn!(
                    "miner address {addr_str} set in {`F3`_PERMANENT_PARTICIPATING_MINER_IDS_ENV_KEY} is not an id address"
                );
                continue;
            };
            ids.insert(id);
...
mod tests {
    ...
    fn test_get_f3_permanent_participating_miner_ids() {
        unsafe {
            // empty
            std::env::set_var(`F3`_PERMANENT_PARTICIPATING_MINER_IDS_ENV_KEY, "");
            assert!(get_f3_permanent_participating_miner_ids().is_none());

            // 1 valid address
            std::env::set_var(`F3`_PERMANENT_PARTICIPATING_MINER_IDS_ENV_KEY, "t01000");
            assert_eq!(
            ...
            // 1 invalid address
            std::env::set_var(`F3`_PERMANENT_PARTICIPATING_MINER_IDS_ENV_KEY, "tf1000");
            assert!(get_f3_permanent_participating_miner_ids().is_none());

            // 1 bls address
            std::env::set_var(
                `F3`_PERMANENT_PARTICIPATING_MINER_IDS_ENV_KEY,
