...
mod test_utils {
    ...
    pub fn call<S, T, U, E>(svc: &mut S, request: T) -> Result<U, E>
    where
        S: Service<T, Response = U, Error = E>,
    {
        futures::executor::block_on(async { svc.ready().await?.call(request).await })
    }
    ...
    pub mod examples {
        ...
        pub async fn len(s: String, method: Option<LenMethod>) -> Result<usize, Error> {
            Ok(match method {
        ...
        /// Method where optional parameter is too soon
        pub async fn bad_len(method: Option<LenMethod>, s: String) -> Result<usize, Error> {
            len(s, method).await
        }
    }
}
