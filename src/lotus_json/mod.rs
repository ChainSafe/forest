...
//! # #[derive(Serialize, Deserialze)] enum Address {}
//! # mod stringify2 {
...
//! - moving implementation to the module where the struct is defined, e.g [crate::blocks::header::lotus_json].
    ...
    ($($mod_name:ident -> $lotus_json_ty:ident for $domain_ty:ty),* $(,)?) => {
        ...
        fn all_quickchecks() {
            $(
                ...
                // ^ make sure the above line is flushed in case the quickcheck fails
