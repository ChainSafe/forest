...
mod test_selection {
    ...
    async fn message_selection_trimming() {
        ...
        let nmsgs = (fvm_shared2::BLOCK_GAS_LIMIT / TEST_GAS_LIMIT) + 1;
        ...
        let expected = fvm_shared2::BLOCK_GAS_LIMIT / TEST_GAS_LIMIT;
        assert_eq!(msgs.len(), expected as usize);
        let mut m_gas_lim = 0;
        for m in msgs.iter() {
            m_gas_lim += m.gas_limit();
        }
        assert!(m_gas_lim <= fvm_shared2::BLOCK_GAS_LIMIT as u64);
    }
    ...
    async fn test_optimal_msg_selection1() {
        ...
        let n_msgs = 10 * fvm_shared2::BLOCK_GAS_LIMIT / TEST_GAS_LIMIT;
        ...
        let expected_msgs = fvm_shared2::BLOCK_GAS_LIMIT / TEST_GAS_LIMIT;
    ...
    async fn test_optimal_msg_selection2() {
        ...
        let n_msgs = 5 * fvm_shared2::BLOCK_GAS_LIMIT / TEST_GAS_LIMIT;
        for i in 0..n_msgs as usize {
            let bias = (n_msgs as usize - i) / 3;
            let m = create_fake_smsg(
                &mpool,
    ...
    async fn test_optimal_msg_selection3() {
        ...
        let n_msgs = 1 + fvm_shared2::BLOCK_GAS_LIMIT / TEST_GAS_LIMIT;
        for i in 0..n_msgs {
            for j in 0..n_actors {
                let premium =
