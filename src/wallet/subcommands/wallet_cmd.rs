...
impl WalletCommands {
    pub async fn run(&self, token: Option<String>) -> anyhow::Result<()> {
        match self {
            ...
            Self::Default => {
                let response = wallet_default_address(Default::default(), &token)
                    .await
                    .map_err(handle_rpc_err)?
            ...
            Self::List {
                ...
                let response = wallet_list(Default::default(), &token)
                    .await
                    .map_err(handle_rpc_err)?
                    .into_inner();

                let default = wallet_default_address(Default::default(), &token)
                    .await
                    .map_err(handle_rpc_err)?;
