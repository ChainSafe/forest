...
impl State {
        ...
        BS: BlockStore,
    {
        if actor.code == *actorv0::POWER_ACTOR_CODE_ID {
            ...
            Ok(store
                ...
                .map(State::V6)
                .ok_or("Actor state doesn't exist in store")?)
        } else if actor.code == *actorv7::POWER_ACTOR_CODE_ID {
            Ok(store
                .get(&actor.state)?
                .map(State::V7)
                .ok_or("Actor state doesn't exist in store")?)
    ...
    pub fn miner_power<BS: BlockStore>(
        ...
        match self {
            ...
            State::V7(st) => Ok(st.miner_power(s, miner)?.map(From::from)),
        }
    }
    ...
    pub fn list_all_miners<BS: BlockStore>(&self, s: &BS) -> Result<Vec<Address>, Box<dyn Error>> {
        match self {
            ...
            State::V7(st) => {
                let claims = actorv7::make_map_with_root(&st.claims, s)?;
                let mut miners = Vec::new();
                claims.for_each(|k, _: &actorv3::power::Claim| {
                    miners.push(Address::from_bytes(&k.0)?);
                    Ok(())
    ...
    pub fn miner_nominal_power_meets_consensus_minimum<BS: BlockStore>(
        ...
        match self {
            ...
            State::V7(st) => st.miner_nominal_power_meets_consensus_minimum(s, miner),
        }
    }
...
impl From<actorv7::power::Claim> for Claim {
    fn from(cl: actorv7::power::Claim) -> Self {
        Self {
            raw_byte_power: cl.raw_byte_power,
            quality_adj_power: cl.quality_adj_power,
        }
