...
macro_rules! forward_instrumented {
    ($method_name:literal, $call:expr) => {{
        let stopwatch = time::Instant::now();
        let result = $call();
        let elapsed = stopwatch.elapsed();

        metrics::KERNEL_OP_COUNT
...
impl<DB: Blockstore> fvm::kernel::ActorOps for ForestInstrumentedKernel<DB> {
    fn resolve_address(&self, address: &Address) -> fvm::kernel::Result<Option<ActorID>> {
        forward_instrumented!("resolve_address", || self.0.resolve_address(address))
    }

    fn get_actor_code_cid(&self, id: ActorID) -> fvm::kernel::Result<Option<Cid>> {
        forward_instrumented!("get_actor_code_cid", || self.0.get_actor_code_cid(id))
    }

    fn new_actor_address(&mut self) -> fvm::kernel::Result<Address> {
        forward_instrumented!("new_actor_address", || self.0.new_actor_address())
    }

    fn create_actor(&mut self, code_id: Cid, actor_id: ActorID) -> fvm::kernel::Result<()> {
        forward_instrumented!("create_actor", || self.0.create_actor(code_id, actor_id))
    }

    fn get_builtin_actor_type(&self, code_cid: &Cid) -> u32 {
        forward_instrumented!("get_builtin_actor_type", || self
            .0
            .get_builtin_actor_type(code_cid))
    }

    fn get_code_cid_for_type(&self, typ: u32) -> Result<Cid> {
        forward_instrumented!("get_code_cid_for_type", || self
            .0
...
impl<DB: Blockstore> fvm::kernel::IpldBlockOps for ForestInstrumentedKernel<DB> {
    fn block_open(&mut self, cid: &Cid) -> fvm::kernel::Result<(BlockId, BlockStat)> {
        forward_instrumented!("block_open", || self.0.block_open(cid))
    }

    fn block_create(&mut self, codec: u64, data: &[u8]) -> fvm::kernel::Result<BlockId> {
        forward_instrumented!("block_create", || self.0.block_create(codec, data))
    }
    ...
    fn block_link(
        ...
        forward_instrumented!("block_link", || self.0.block_link(id, hash_fun, hash_len))
    }

    fn block_read(&mut self, id: BlockId, offset: u32, buf: &mut [u8]) -> fvm::kernel::Result<i32> {
        forward_instrumented!("block_read", || self.0.block_read(id, offset, buf))
    }

    fn block_stat(&mut self, id: BlockId) -> fvm::kernel::Result<BlockStat> {
        forward_instrumented!("block_stat", || self.0.block_stat(id))
    }
}
impl<DB: Blockstore> fvm::kernel::CircSupplyOps for ForestInstrumentedKernel<DB> {
    fn total_fil_circ_supply(&self) -> fvm::kernel::Result<TokenAmount> {
        match self.1.clone() {
            ...
            None => {
                forward_instrumented!("total_fil_circ_supply", || self.0.total_fil_circ_supply())
            }
        }
    }
}
...
impl<DB: Blockstore> fvm::kernel::CryptoOps for ForestInstrumentedKernel<DB> {
    fn hash(&mut self, code: u64, data: &[u8]) -> Result<cid::multihash::MultihashGeneric<64>> {
        forward_instrumented!("hash", || self.0.hash(code, data))
    }
    ...
    fn compute_unsealed_sector_cid(
        ...
        forward_instrumented!("compute_unsealed_sector_cid", || self
            .0
    ...
    fn recover_secp_public_key(
        ...
        forward_instrumented!("recover_secp_public_key", || self
            .0
...
impl<DB: Blockstore> DebugOps for ForestInstrumentedKernel<DB> {
    fn log(&self, msg: String) {
        forward_instrumented!("log", || self.0.log(msg))
    }

    fn debug_enabled(&self) -> bool {
        forward_instrumented!("debug_enabled", || self.0.debug_enabled())
    }

    fn store_artifact(&self, name: &str, data: &[u8]) -> Result<()> {
        forward_instrumented!("store_artifact", || self.0.store_artifact(name, data))
    }
}
impl<DB: Blockstore> GasOps for ForestInstrumentedKernel<DB> {
    /// Returns the gas used by the transaction so far.
    fn gas_used(&self) -> Gas {
        forward_instrumented!("gas_used", || self.0.gas_used())
    }

    /// Returns the remaining gas for the transaction.
    fn gas_available(&self) -> Gas {
        forward_instrumented!("gas_available", || self.0.gas_available())
    }

    /// `charge_gas` charges specified amount of `gas` for execution.
    /// `name` provides information about gas charging point.
    fn charge_gas(&mut self, name: &str, compute: Gas) -> Result<()> {
        forward_instrumented!("charge_gas", || self.0.charge_gas(name, compute))
    }

    /// Returns the currently active gas price list.
    fn price_list(&self) -> &PriceList {
        forward_instrumented!("price_list", || self.0.price_list())
    }
}
impl<DB: Blockstore> MessageOps for ForestInstrumentedKernel<DB> {
    fn msg_caller(&self) -> ActorID {
        forward_instrumented!("msg_caller", || self.0.msg_caller())
    }

    fn msg_receiver(&self) -> ActorID {
        forward_instrumented!("msg_receiver", || self.0.msg_receiver())
    }

    fn msg_method_number(&self) -> MethodNum {
        forward_instrumented!("msg_method_number", || self.0.msg_method_number())
    }

    fn msg_value_received(&self) -> TokenAmount {
        forward_instrumented!("msg_value_received", || self.0.msg_value_received())
    }
}
impl<DB: Blockstore> NetworkOps for ForestInstrumentedKernel<DB> {
    fn network_epoch(&self) -> ChainEpoch {
        forward_instrumented!("network_epoch", || self.0.network_epoch())
    }

    fn network_version(&self) -> fvm_shared::version::NetworkVersion {
        forward_instrumented!("network_version", || self.0.network_version())
    }

    fn network_base_fee(&self) -> &TokenAmount {
        forward_instrumented!("network_base_fee", || self.0.network_base_fee())
    }
}
impl<DB: Blockstore> RandomnessOps for ForestInstrumentedKernel<DB> {
    fn get_randomness_from_tickets(
        ...
        forward_instrumented!("get_randomness_from_tickets", || self
            .0
    ...
    fn get_randomness_from_beacon(
        ...
        forward_instrumented!("get_randomness_from_beacon", || self
            .0
...
impl<DB: Blockstore> SelfOps for ForestInstrumentedKernel<DB> {
    fn root(&self) -> Result<Cid> {
        forward_instrumented!("root", || self.0.root())
    }

    fn set_root(&mut self, root: Cid) -> Result<()> {
        forward_instrumented!("set_root", || self.0.set_root(root))
    }

    fn current_balance(&self) -> Result<TokenAmount> {
        forward_instrumented!("current_balance", || self.0.current_balance())
    }

    fn self_destruct(&mut self, beneficiary: &Address) -> Result<()> {
        forward_instrumented!("self_destruct", || self.0.self_destruct(beneficiary))
    }
}
impl<DB: Blockstore> SendOps for ForestInstrumentedKernel<DB> {
    fn send(
        ...
        forward_instrumented!("send", || self.0.send(recipient, method, params, value))
    }
}
