...
impl State
{
    ...
    pub fn put_verified<BS: BlockStore>(&mut self, store: &BS,verified_addr : Address, verifier_cap : Datacap) ->Result<(), String>
    {
        Self::put(&mut self.verifiers,store,verified_addr,verifier_cap)
    }

    pub fn get_verifier<BS: BlockStore>(&mut self,store: &BS,address_get:Address) ->Result<Option<Datacap>,String>
    {
        Self::get(&mut self.verifiers,store,address_get)
    }

    pub fn delete_verifier<BS: BlockStore>(&mut self,store: &BS,address:Address) ->Result<(),String>
    {
        Self::delete(&mut self.verifiers,store,address)
    }

    pub fn put_verified_client<BS: BlockStore>(&mut self, store: &BS,verified_addr : Address, verifier_cap : Datacap) ->Result<(), String>
    {
        Self::put(&mut self.verified_clients,store,verified_addr,verifier_cap)
    }

    pub fn get_verified_clients<BS: BlockStore>(&mut self,store: &BS,address:Address) ->Result<Option<Datacap>,String>
    {
        Self::get(&mut self.verified_clients,store,address)
    }

    pub fn delete_verified_clients<BS: BlockStore>(&mut self,store: &BS,address:Address) ->Result<(),String>
    {
    ...
    fn put<BS: BlockStore>(storage : &mut Cid,store: &BS,verified_addr : Address, verifier_cap : Datacap) ->Result<(), String>
    {
    ...
    fn delete<BS: BlockStore>(storage : &mut Cid,store: &BS,verified_addr : Address) ->Result<(), String>
    {
        let mut map: Hamt<BytesKey, _> = Hamt::load_with_bit_width(&storage, store, HAMT_BIT_WIDTH)?;
