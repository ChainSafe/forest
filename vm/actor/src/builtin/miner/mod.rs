...
use crate::{account::Method as AccountMethod, actor_error, market::{self, ActivateDealsParams, ComputeDataCommitmentReturn, SectorDataSpec}, power::MAX_MINER_PROVE_COMMITS_PER_EPOCH};
use crate::{
...
impl Actor {
        ...
        RT: Runtime<BS>,
    {
        ...
        if params.aggregate_proof.len() < MAX_AGGREGATED_PROOF_SIZE {
            return Err(actor_error!(
                ...
                params.aggregate_proof.len(), MAX_AGGREGATED_PROOF_SIZE
            ))
        }
        let store = rt.store();
        let state: State = rt.state()?;
        let precommits = state.get_all_precommitted_sectors(store, sector_numbers)
        .map_err(|e|{
        ...
        for (i, precommit) in precommits.iter().enumerate() {
            let msd = max_prove_commit_duration(precommit.info.seal_proof)
            .ok_or_else(|| actor_error!(ErrIllegalState, "no max seal duration for proof type: {}", i64::from(precommit.info.seal_proof)))?;
            let prove_commit_due = precommit.pre_commit_deposit.clone() + msd;
            if BigInt::from(rt.curr_epoch()) > prove_commit_due {
                return Err(actor_error!(
                    ErrIllegalArgument,
                    "commitment proof for {} too late at {}, due {}", precommit.info.sector_number, rt.curr_epoch(), prove_commit_due
                ));
            }
            ...
            if i >= 1 {
                ...
                if prev_seal_proof != precommit.info.seal_proof {
                    return Err(actor_error!(
                        ErrIllegalState,
                        "aggregate contains mismatched seal proofs {} and {}", i64::from(prev_seal_proof), i64::from(precommit.info.seal_proof)
                    ))
                }
            }
        ...
        for (i, precommit) in precommits.iter().enumerate() {
            ...
            if rt.curr_epoch() <= interactive_epoch {
                return Err(actor_error!(
                    ErrForbidden, "too early to prove sector {}", precommit.info.sector_number
                ));
            }
            let sv_info_randomness = rt.get_randomness_from_tickets(SealRandomness, precommit.info.seal_rand_epoch, &receiver_bytes)?;
            let sv_info_interactive_randomness = rt.get_randomness_from_beacon(InteractiveSealChallengeSeed, interactive_epoch,&receiver_bytes)?;
        ...
        if precommits.len() <= 0 {
            return Err(actor_error!(ErrIllegalState,"bitfield non-empty but zero precommits read from state"));
        }
        rt.verify_aggregate_seals(&AggregateSealVerifyProofAndInfos {
            ...
            actor_error!(ErrIllegalArgument, "aggregate seal verify failed: {}", e)
        })?;
        ...
        RT: Runtime<BS>,
    {
        ...
        let precommited_sectors = st.find_precommitted_sectors(store, &params.sectors).map_err(|e| e.downcast_default(
            ExitCode::ErrIllegalState, "failed to load pre-committed sectors"))?;
    ...
    RT: Runtime<BS>,
{
    ...
    let commds = request_unsealed_sector_cids(rt,
        &[SectorDataSpec{
            deal_ids: params.deal_ids.clone(),
    ...
    RT: Runtime<BS>,
{
    ...
    let ret: ComputeDataCommitmentReturn = rt.send(
        *STORAGE_MARKET_ACTOR_ADDR,
