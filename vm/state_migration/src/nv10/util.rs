...
>(
    ...
    let in_root_node = Array::load(&root, store, ActorVersion::V2)
        .map_err(|e| MigrationError::AMTLoad(e.to_string()))?;
...
>(
    ...
    let in_v2_root_node_outer = Map2::load(&root, store, ActorVersion::V2)
        .map_err(|e| MigrationError::HAMTLoad(e.to_string()))?;

    let mut out_v3_root_node_outer = make_empty_map(store, new_outer_bitwidth as u32); // FIXME check cast

    in_v2_root_node_outer
        .for_each(|k: &BytesKey, v: &Cid| {
