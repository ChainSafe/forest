...
pub async fn new(mut client: RawClient<HTC>, sig_type: Vec<u8>) -> Result<String, JsonRpcError> {
        Ok(Filecoin::wallet_new(&mut client, sig_type)
        .await?)
}
...
pub async fn balance(mut client: RawClient<HTC>, address: String) -> Result<String, JsonRpcError> {
        Ok(Filecoin::wallet_balance(&mut client, address)
        .await?)
}

/// Marks the given address as as the default one
pub async fn set_default(mut client: RawClient<HTC>, address: String) -> Result<(), JsonRpcError> {
        Ok(Filecoin::wallet_set_default(&mut client, address)
        .await?)
}
...
pub async fn sign(mut client: RawClient<HTC>, params: (String, String)) -> Result<SignatureJson, JsonRpcError> {
        Ok(Filecoin::wallet_sign(&mut client, params)
        .await?)
}
...
pub async fn import(mut client: RawClient<HTC>, key_info: KeyInfoJson) -> Result<String, JsonRpcError> {
        Ok(Filecoin::wallet_import(&mut client, key_info)
        .await?)
}

/// Returns the private key of an address in the wallet
pub async fn export(mut client: RawClient<HTC>, address: String) -> Result<KeyInfoJson, JsonRpcError> {
        Ok(Filecoin::wallet_export(&mut client, address)
        .await?)
}