...
const DEFAULT_ALPHA : usize = 0;
const DEFAULT_BETA : usize = 1;
...
pub fn get_const_bigs() -> Vec<BigInt> {
    parse(&CONST_STRS[..]).unwrap()
}
...
pub fn get_cum_sum_ratio_epsilon() -> BigInt {
    BigInt::parse_bytes(CONST_STRS[EXTRAPOLATED_CUM_SUM_RATIO_EPSILON].as_bytes(), 10).unwrap()
}

#[test]fn parse_check () {
    let _ = get_ln_num_coef();
    let _ = get_ln_denom_coef();
    let _ = get_const_bigs();
