        ...
        match self {
            ...
            Self::List {
                ...
                for address in response {
                    ...
                    let balance_string = match (exact_balance, fixed_unit) {
                        // absolute, no rounding
                        (true, true) => format!("{:#}", balance_token_amount.pretty()),
                        // absolute, with rounding
                        (true, false) => format!("{:#.4}", balance_token_amount.pretty()),
                        // with units, no rounding
                        (false, true) => format!("{}", balance_token_amount.pretty()),
                        // with units, rounded
