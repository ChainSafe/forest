...
Once a `FullTipset` struct is acquired, it's handed over to the `SyncStateMachine` struct. This is the core of the `ChainFollower`, managing all candidate tipsets and deciding what to do next.
