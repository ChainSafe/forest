...
use fil_actors_runtime_v8::{parse_uint_key, Map, MessageAccumulator};
...
fn check_miner_balances<BS: Blockstore>(
    ...
    acc.require(balance - &state.locked_funds - &state.pre_commit_deposits - &state.initial_pledge >= TokenAmount::zero(), format!("miner balance {balance} is less than sum of locked funds ({}), precommit deposit ({}) and initial pledge ({})", state.locked_funds, state.pre_commit_deposits, state.initial_pledge));
