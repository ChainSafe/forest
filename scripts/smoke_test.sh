...
OUTPUT=$(curl --write-out "WalletDefault %{http_code}\n" -s -X POST -H 'Content-Type: application/json-rpc' -H 'Authorization: Bearer ${TOKEN}' -d '{"jsonrpc": "2.0", "method":"Filecoin.WalletList", "params":[], "id": 0}' http://127.0.0.1:1235/rpc/v0)
