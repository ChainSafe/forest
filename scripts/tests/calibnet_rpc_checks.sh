...
# Run forest-tool RPC server at 2346
"$FOREST_TOOL_PATH" api serve "$EXPORTED_SNAPSHOT" "$old_snapshot" --chain calibnet --port 2346 &
FOREST_TOOL_PID=$!
...
temp_dir=$(mktemp -d)
FULLNODE_API_INFO="/ip4/127.0.0.1/tcp/2346/http" forest-cli chain block -c bafy2bzacecpjvcld56dazyukvj35uzwvlh3tb4ga2lvbgbiua3mgbqaz45hbm | jq . > "$temp_dir/block.json"
...
$FOREST_TOOL_PATH api compare "$EXPORTED_SNAPSHOT" --forest /ip4/127.0.0.1/tcp/2346/http --lotus /ip4/127.0.0.1/tcp/1234/http --n-tipsets 5 --filter-file api_compare/filter-list || true
...
# forest-tool vs lotus daemon
$FOREST_TOOL_PATH api compare "$EXPORTED_SNAPSHOT" --forest /ip4/127.0.0.1/tcp/2346/ws --lotus /ip4/127.0.0.1/tcp/1234/ws --n-tipsets 5 --filter-file api_compare/filter-list || true
