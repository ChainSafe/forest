...
echo "bootstrap_peers = [\"$ADDRESS\"]" >> $CONFIG_PATH

NODE_LOG_DIRECTORY=$(mktemp --directory)
RUST_LOG="info,forest_filecoin::chain_sync::network_context=debug" $FOREST_PATH --chain calibnet --encrypt-keystore false --config "$CONFIG_PATH" --no-metrics --rpc false --log-dir "$NODE_LOG_DIRECTORY" &
PID=$!
